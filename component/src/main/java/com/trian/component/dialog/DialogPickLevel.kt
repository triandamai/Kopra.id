...
fun DialogPickLevel(
    ...
    show:Boolean,
    onPick:(levelUser:LevelUser)->Unit,
    onCancel:()->Unit
) {

        val listStatus = listOf("Petani","Pengepul","Penyewa Kendaraan")
        var mRememberStatusUser by remember{ mutableStateOf(LevelUser.UNKNOWN) }
        var sizeBorder by remember{ mutableStateOf(0.dp) }
        var colorBorder by remember{ mutableStateOf(Color.Unspecified) }

    fun getLevelByString(level:String):LevelUser{
        return when(level){
            "Petani"->LevelUser.FARMER
            "Pengepul"->LevelUser.COLLECTOR
            "Penyewa Kendaraan"->LevelUser.TENANT
            else->LevelUser.UNKNOWN
        }
    }
                        ...
                        items(count = listStatus.size,itemContent = {index->
                            Card(
                                ...
                                modifier = modifier
                                    ...
                                    .padding(top = 8.dp)
                                    .selectable(
                                        selected = mRememberStatusUser == getLevelByString(listStatus[index]),
                                        onClick = {
                                            mRememberStatusUser =getLevelByString(listStatus[index])
                                        }
                                    ),
                                elevation = 0.dp,
                                ...
                                Row(
                                    ...
                                    RadioButton(
                                        selected = mRememberStatusUser == getLevelByString(listStatus[index]),
                                        onClick = { mRememberStatusUser = getLevelByString(listStatus[index]) },
                                        enabled = true,
